...
use windmill_parser_sql::{
    ...
    parse_starrocks_sig, RE_ARG_MYSQL_NAMED,
};
...
// StarRocks does not support COM_STMT_PREPARE (MySQL binary protocol). Interpolate
// named/positional params directly into the query string so we can use text-protocol
// query methods (COM_QUERY) instead of exec methods (COM_STMT_PREPARE + COM_STMT_EXECUTE).
fn interpolate_mysql_params(query: &str, params: &Params) -> String {
    match params {
        Params::Named(m) if !m.is_empty() => {
            let mut result = query.to_string();
            // Sort by length descending so :foobar is replaced before :foo
            let mut pairs: Vec<_> = m.iter().collect();
            pairs.sort_by(|a, b| b.0.len().cmp(&a.0.len()));
            for (name, value) in pairs {
                let placeholder = format!(":{}", String::from_utf8_lossy(name));
                // as_sql(false) = escape backslashes (MySQL-compatible mode)
                result = result.replace(&placeholder, &value.as_sql(false));
            }
            result
        }
        Params::Positional(v) if !v.is_empty() => {
            let mut result = String::new();
            let mut iter = v.iter();
            for ch in query.chars() {
                if ch == '?' {
                    if let Some(val) = iter.next() {
                        result.push_str(&val.as_sql(false));
                    } else {
                        result.push(ch);
                    }
                } else {
...
) -> windmill_common::error::Result<BoxFuture<'a, windmill_common::error::Result<Vec<Box<RawValue>>>>>
{
    ...
    let query = interpolate_mysql_params(query, &statement_values);
    ...
    let result_f = async move {
        if skip_collect {
            ...
            let rows_stream = async_stream::stream! {
                ...
                let mut result = match conn.query_iter(&query).await.map_err(to_anyhow) {
                    Ok(result) => result,
                    Err(e) => {
...
pub async fn do_starrocks(
    ...
    let database = if let Some(db) = db_arg {
        serde_json::from_value::<StarRocksDatabase>(db)
    ...
    let opts = OptsBuilder::default()
        ...
        .tcp_port(database.port.unwrap_or(9030))
        // StarRocks doesn't implement the MySQL @@socket system variable; disable
        // socket discovery to prevent mysql_async from querying it on local connections.
        .prefer_socket(false);
    ...
    let sig = parse_starrocks_sig(query)
...
fn convert_mysql_value_to_json(v: mysql_async::Value, c: ColumnType) -> serde_json::Value {
    return match v {
        ...
        mysql_async::Value::Bytes(b) if c.is_character_type() => {
            // PostgreSQL JSONB rejects   (null bytes); strip them.
            // CHAR columns in particular may carry null-byte padding.
